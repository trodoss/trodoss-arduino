...
{  
  ...
  switch (game_state)
  {
	...
	case GAME_PLAYING:
       if (TV.millis() >= next_action)
       {      
         if (Controller.upPressed()) moveElf(FACING_UP);
         if (Controller.downPressed()) moveElf(FACING_DOWN);
         if (Controller.rightPressed()) moveElf(FACING_RIGHT);
         if (Controller.leftPressed()) moveElf(FACING_LEFT);
         if (Controller.firePressed()) throwSword();
